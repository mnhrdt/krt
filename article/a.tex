...
\begin{abstract}
	We propose a generalization of the classical rank transform of
	image processing, where the square neighborhood is replaced by an
	arbitrary weight kernel.  We prove the main structural properties of this
	operator: it is contrast-invariant, its infinitesimal generator is the
	curvature of level lines, and it has a noise-uniformization property: it
	transforms white noise into uniform noise.  Since contrast-invariant
	operators are necessarily discontinuous, we provide a continuous
	version of the operator that is numerically invariant to contrast changes
	of limited slope.
	We discuss several applications, and its relationship to other operations:
	noise whitening transforms, local contrast equalization, bilateral
	filtering
\end{abstract}
...
variable~$K_p$ is discrete uniform with values between~$0$ and~$N$,
...
Observation: this formula is valid for~$A\neq B$.  When~$A=B$ it gives the
wrong result.  The correct result is~$\frac1{12}$ (the variance of the
uniform distribution), and this must be computed separately.

\clearpage

\begin{proposition}
	%Let~$\kappa_i=1$ for~$i=1,\ldots,N$.
	Let~$\{X_i\}$ be a set of independent, identically distributed random
	variables with density.
	Then the law of the random variable
	\[
		K_p = \sum_{i\in\mathcal N}H(X_p-X_{p+1})
	\]
	is uniform~$P(K_p=t)=\frac1{N+1}$ for~$t=0,\ldots,N$.
\end{proposition}

\begin{proof}
	Let us consider the sorted pixel values
	\[
		X_{(0)} < X_{(1)} < \cdots < X_{(N)}
	\]
	by the hypotheses, all the~$(N+1)!$ possible permutations  have the same
	probability.
	The event~$K_p=t$ corresponds to~$X_{(t)}=X_p$, which happens in~$N!$ of
	the possible permutations (which are all the permutations of the
	remaning~$N$ variables).  Thus~$P(K_p=t)=\frac{N!}{(N+1)!}=\frac1{N+1}$.
\end{proof}


\begin{proposition}
	Let~$\{\kappa_i\}$ be a set of~$N$ numbers such that the sums~$\sum_{i\in
	A}\kappa_i$ are different for each subset~$A\subseteq\{1,\ldots,N\}$.
	Let~$\{X_i\}$ be a set of independent, identically distributed random
	variables with density.
	Then the law of the random variable
	\[
		K_p = \sum_{i\in\mathcal N}\kappa_i H(X_p-X_{p+1})
	\]
	is given by
	\[
		P\left(K_p=\sum_{i\in A}\kappa_i\right)
		=
		\frac1{(N+1){N\choose |A|}}
	\]
\end{proposition}

\begin{proposition}
	In the setup of the previous proposition, let~$F(t)=P(K_p<t)$ be the
	cumulative distribution function of~$K_p$.  Then the error between~$F$ and
	a uniform distribution is bounded by the largest~$\kappa_i$.  More precisely
	\[
		\left|
		F(t)-\frac{t}{\sum\kappa_i}
		\right|
		<\frac{\max\kappa_i}{\sum\kappa_i}
	\]
\end{proposition}
...
\subsection{Continuous noise model}

It is interesting to rewrite the computations above for the case of a
continuous domain.  Thus the white noise image is a white Gaussian random
field~$\mathcal{W}$.  This field is not a function in the classical sense
--we cannot evaluate~$\mathcal{W}$ at a point~$p$ and say
that~$\mathcal{W}(p)$ is a normal variable.  It is instead a distribution
that we can integrate against test functions~$\varphi$, to obtain
well-defined integrals~$\int\mathcal{W}\varphi$ which are random variables
of known distributions.  See~LeGall2016 for a detailed explanation of this
object.  More precisely, the
distribution~$\mathcal{W}$ is determined by the following properties

$$\int\mathcal{W}\varphi\ \sim\ N\left(0,\int\varphi^2\right)$$

and, as a consequence~$\varphi*\mathcal{W}$ is well defined and

$$\varphi*\mathcal{W}(p) \sim\ N\left(0,\int\varphi^2\right)$$

for all~$p\in\mathbf{R}^2$.

And

$$\mathrm{cov}\left(\mathcal{W}_\varphi(p),\mathcal{W}_\varphi(q)\right)=
\int\varphi(x - p)\varphi(x-q) d x
=\varphi\star\varphi(p-q)$$

As a consequence, if~$\mathrm{supp}(\varphi)\subseteq B_t(0)$ then the
variables~$\varphi*\mathcal{W}(p)$ and~$\varphi*\mathcal{W}(q)$ are
independent when~$\|p-q\|<\frac12t$.

This allows to compute the expectation
of~$I_\epsilon(0)=KRT_{\kappa,H}(\mathcal{W}_\epsilon)(0)$
as

$$
\int_{\mathbf{R}^2}\kappa(-q)H(
	\mathcal{W}_\epsilon(0)
	-
	\mathcal{W}_\epsilon(q)
)dq
=
\int_{|q|<\epsilon}
\kappa(-q)H(
...
and the expectation inside the integral is~$\frac12$, thus it
can be seen the limit as~$\epsilon\to0$ is
$\frac12\int\kappa$.  NOTE: there are maybe regularity hypotheses on~$\kappa$, for
example that it is smooth enough around~$0$.

\bigskip

Let's try to compute the covariance between the values of the KRT
of~$\epsilon$-filtered white gaussian noise at two
points~$p,q$, namely

\[
	\mathrm{cov}(
		KRT_{\kappa,H}(\mathcal{W}_\epsilon)(p),
		KRT_{\kappa,H}(\mathcal{W}_\epsilon)(q)
	)
\]
\[
	= E(
		KRT_{\kappa,H}(\mathcal{W}_\epsilon)(p)\cdot
		KRT_{\kappa,H}(\mathcal{W}_\epsilon)(q)
	)
	)
\]
\[
	= E\left(
		\int\kappa(p-x)H(
			\mathcal{W}_\epsilon(p)
			-
			\mathcal{W}_\epsilon(x)
		)dx
		\cdot
		\int\kappa(q-y)H(
			\mathcal{W}_\epsilon(q)
			-
			\mathcal{W}_\epsilon(y)
		)dy
	\right)
\]
\[
...
We are interested in the limit of this expression,  when~$\epsilon\to 0$.
We will see that the limit exists and it's a function of~$(p,q)$ and the
kernel~$\kappa$.  Let us name it
\[
	\mathrm{cov}(K_p,K_q)
	:=
	\lim_{\epsilon\to0}
	...
	\bigr)
\]
To deal with the limit, notice that for any~$(p,q)$ the
function~$(x,y)\mapsto\kappa(p-x)\kappa(q-y)$ is integrable in~$R^2\times
R^2$ and that the expectation is pointwise bounded by~$1$.  Thus by
dominated convergence we can move the limit inside the integral,
where~$(p,q,x,y)$ are constant.

We start with the easier computation for the case when~$p=q$, which will
give the variance of the~$KRT$ at one point.
\[
	\mathrm{cov}(K_p,K_p)
	=
	\lim_{\epsilon\to0}
	...
	\right)
		...
		dy
\]
to compute this integral, we divide the domain of integration into two parts
\begin{equation}
	\label{eq:domainxy}
	\lim_{\epsilon\to0}
	\left(
		\iint_{\begin{smallmatrix}
			|p-x|>\epsilon\\
			|p-y|>\epsilon\\
			|x-y|>\epsilon
		\end{smallmatrix}}\cdots
	+\int_{\textrm{elsewhere}}\cdots
	\right)
\end{equation}
The first integral can be computed explicitly, for the separation conditions
assure that the three values at~$p,x,y$ are independent variables.  And we
know that if~$A,B,C$ are independent variables
then~$E(H(A>B)H(A>C))=\frac13$.  The rest of the integral concerns a region
of the space (see Figure~\ref{fig:domainxy}) of vanishing width, whose
integrals with respect to~$\kappa$ will go to zero.
...
\[
	\mathrm{cov}(K_p,K_p)
	=
	\frac13\iint\kappa(p-x)\kappa(p-y)dxdy
	=
	\frac13\left(\int\kappa\right)^2
\]


The computation of~$\mathrm{cov}(K_p,K_p)$ uses a similar idea:
...
now, inside the first integral, the expectation
\[
	E\left(
		H(
			\mathcal{W}_\epsilon(p)
	...
	\right)
\]
involves four independent variables (since all the involved points are
further than~$\epsilon$), thus the expectation equals~$\frac14$ and the
covariance equals~$\frac14-\frac14=0$.
...
\begin{figure}
	\centering
	\includegraphics[width=0.5\linewidth]{f/domain.jpg}
	\caption{
		\label{fig:domainxy}
		The blue part of the plain represents the domain of integration of the
		first integral in equation~\ref{eq:domainxy}.
	}
\end{figure}
