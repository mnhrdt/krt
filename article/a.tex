...
\begin{proposition}[structural properties of~$R_{\kappa,\sigma}$]
	...
	\begin{enumerate}
		...
		\item[\bf P5] If~$\varphi\in O(2)$ is a rigid motion
			of~$\mathbf{R}^2$
			then~$R_{\kappa,\sigma}\left(u\circ\varphi\right) =
			\left(R_{\kappa,\sigma} u\right)\circ\varphi$.
	\end{enumerate}
\end{proposition}
...
\begin{definition}
	An~\emph{isotropic kernel} is a function~$g:\mathbf{R}^2\to\mathbf{R}$
...
\begin{proposition}
	...
	\[
		\lim_{h\to0} R_{g_h}(u)(x) = \frac12
	\]
	and
	\[
		\lim_{h\to0} \frac{R_{g_h}(u)(x)-\frac12}h =
	\frac13\mathrm{curv}\left(u(x))\right)
	\]
...
\begin{proposition}
	...
	x=\gamma<+\infty$.  Then
	\[
		K_{g_h}(u)(0)=\frac12+\frac{\kappa}2\sigma^2h+O(h^2)
	\]
\end{proposition}

\begin{proof}
	Let us write the definition of~$K_{g_h}(u)(0)$:
	\[
		K_{g_h}(u)(0)=
...
\begin{proposition}
	...
	centered kernel such that~$\int_{-\infty}^\infty x^2g(x,0)\mathrm{d}
	x=\sigma^2<+\infty$ and~$\int_{-\infty}^\infty x^4g_y(x,0)\mathrm{d}
	x=\gamma<+\infty$.  Then, if we
	denote~$g_h(x,y)=\frac1{h^2}g(x/h,y/h)$,
	\[
		K_{g_h}(u)(0)=\frac12+\frac{c''(0)}2\sigma^2h^2+O(h^3)
	\]
\end{proposition}
...
\begin{proof}
	...
	\[
		=
		\int_{-\infty}^\infty\int_{-\infty}^{c\left(hx\right)}
		g\left(x,y\right)\ %
		\mathrm{d}y
		\mathrm{d}x
	\]
	...
	\[
		f'(h)=
		\int_{-\infty}^\infty xc'(hx)
		\left[\vphantom{\int}g(x,y)\right]_{-\infty}^{c(hx)}
		\ \mathrm{d}x
		=
		\int_{-\infty}^\infty xc'(hx)
		g\left(x,c\left(hx\right)\right)
		\ \mathrm{d}x
	\]
	thus
	\[
	...
	\[
		...
		\int_{-\infty}^\infty
		\left(
			x^2c''(hx)g(x,c(hx))+\left(xc'(hx)\right)^2g_y(x,c(hx))
		\right)
		%f''(h)=
		%\frac{\kappa}2
		%\int_{-\infty}^\infty \frac{\kappa x^2}2
		...
		%<\gamma
	\]
	thus~$f''(0)=c''(0)\int x^2g(x,0)\mathrm{d}x$ and we can write
	\[
		K_{g_h}(u)(0)
		=
		\frac12
		+
		\frac{c''(0)\sigma^2}2h^2+\cdots
	\]
	to see that the remaining terms are~$O(h^3)$ we need to
	compute~$f'''$ and bound it (possibly requiring further conditions
	on~$c$).
\end{proof}

NOTE: compare this result with the previous one.
...
%	\lim_{h\to0}
%	R_{\kappa_h}(u)(x)-u(x)
%	=h^qc_\kappa\mathrm{div}\left(\frac{\nabla u}{\left\|{\nabla
%	u}\right\|}\right)
%\]
%
%where
%\[
%	\kappa_h(x)=\frac{1}{h^2p}\kappa\left(\frac{x}{h^p}\right)
%\]
