...
\begin{remark}
	...
	laplacian-consistent when~$M_0(g)=1$, $M_1(g)=0$, $M_2(g)=\rho^2I_2$
	and~$M_3(g)<+\infty$.
\end{remark}
...
\begin{proof}
	...
	obtain~$u(x)+0+\frac{\rho^2}{2}\Delta u(x)$.  The remaining term can be
...
\begin{proposition}[Local effect of KRT with continuous step]
	...
	Let~$s:\mathbf{R}\to\mathbf{R}$ be a~$C^2$ function with bounded
	derivatives up to order~$3$.
	Then
	\[
		\textsc{krt}_{g_h,s}(u)(x) = s(0)
		+
		\frac{
			s'(0)\Delta u(x)
			+
...
\end{proposition}
\begin{proof}
	The result is obtained by linearity of~$\textsc{krt}_{g_h,s}(u)$ with
	respect to~$s$ and the Taylor expansion of~$s$:
	\[
		s(r)=s(0)+s'(0)r+\frac{s''(0)}2r^2+\frac{s'''(\xi)}{3!}r^3
	\]
...
Missing proposition: local effect of KRT with Heaviside step when the level
line has a kink or angle.
...
\begin{proof}
	...
	previous proposition is equal to~$\frac{\gamma}{2\pi}$.  Then, the
	proposition reads
	\[
