...
\begin{proposition}[structural properties of~$R_{\kappa,\sigma}$]
	...
	\begin{enumerate}
		...
		\item[\bf P5] If~$\varphi(x)=Ax+b$ is an affine map
			...
			then\\
			$\displaystyle
			R_{\kappa,\sigma}\left(u\circ\varphi\right) =
			R_{\frac1{|A|}\kappa\circ
...
\begin{proposition}
	...
	\begin{enumerate}
		...
		\item[\bf H2] If~$x$ is the site of the strict global
			maximum of~$u$,
			then~$R_{\kappa,H}(u)(x)=\int\kappa$.  This is also
		...
		\item[\bf H3] If~$x$ is the site of the strict global
			minimum of~$u$, then~$R_{\kappa,H}(u)(x)=0$.  This
...
\begin{proposition}[Local effect of KRT with continuous step]
	...
	Then
	\[
		R_{g_h,s}(u)(x) = s(0) + \frac{s'(0)\sigma^2}2\Delta u(x)h^2
		+O(h^3)
	\]
\end{proposition}
...
\begin{proposition}
	...
	\[
		K_{g_h}(u)(0)=\frac12+\frac{c''(0)}2\sigma^2h+O(h^2)
	\]
\end{proposition}
...
\begin{theorem}[behavior of the KRT at smooth points]
	...
	\[
		K_{g_h}(u)(x)=\frac12+\kappa(u,x)\gamma(g)h+O(h^2)
	\]
...
\begin{proof}
	...
	that~$\left|K_{g^M}(u)(x)-K_g(u)(x)\right|<\epsilon$.  (This follows
