...
\begin{proposition}\label{prp:curv1}
	Let~$u:\mathbf{R}^2\to\mathbf{R}$ such that the level
...
\begin{theorem}[behavior of the KRT at smooth points]
	...
	\[
		\textsc{krt}_{g_h,H}(u)(x)=\frac12+\frac1\pi\kappa(u,x)\gamma(g)h+O(h^2)
	\]
	where~$\gamma(g)=\int_{\mathbf{R}^2}\left|g(x)\right|\|x\|dx$ and
...
\begin{proof}
	...
	Explain the plan: we will extend the function~$u$ outside of~$B_R(0)$ so
	that the level line is the graph of a function~$\mathbr{R}\to\mathbf{R}$,
	and we will see that this leads to a controlled approximation of the KRT,
	upon which we will apply the previous result.

	Now,
	we will prove that we can approximate the effect of the kernel~$g_h$ by
	the finite-support kernel~$g_h^R=\mathbf{1}_{B_R(0)}g_h$.  Indeed, the
	...
	Since~$\mathrm{supp}\left(g^R_h\right)\subseteq B_R(0)$,
	by property~$P6$, the values of~$u$ outside~$B_R(0)$ do not matter,
	and~$u$ can be extended in an arbitrary way to satisfy the hypotheses of
	proposition~\ref{prp:curv1}.  Applying this proposition to any such
	extended~$u$ and~$g_h^R$, we have

	To complete the argument: deal with the following four terms, that happen
	to be close to each other (thanks to triangle inequality):\\
	$ \textsc{krt}_{g_h,H}(u)(x) $\\
	$ \textsc{krt}_{g^R_h,H}(u)(x) $\\
	$ \textsc{krt}_{g_h,H}(\tilde u)(x) $\\
	$ \textsc{krt}_{g^R_h,H}(\tilde u)(x) $\\
	where~$\tilde u$ is the function~$u$ extended outside of~$B_R(x)$ in such
	a way that the level curve through~$x$ is the graph of a function~$c$.
